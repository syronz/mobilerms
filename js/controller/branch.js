diakoApp.controller('branchController',function($scope,$rootScope,$http,$log,$location,$routeParams,mainFactory,$indexedDB){

	$scope.objects = [];
	mainFactory.checkLogin();

	$scope.branchList = null;
	$rootScope.part = 'branch';
	...
	$scope.list = function(){
		...
		$http.get("control/branchCtrl.php?action=list&order="+order+"&limit="+limitPage+"&search="+str).
		success(function(data, status, headers, config) {
			$scope.branchList = data.rows;
	...
	$scope.saveModal = function(){
		if($scope.modalType == 'add'){
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/branchCtrl.php?action=add",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
			...
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/branchCtrl.php?action=edit",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
			...
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/branchCtrl.php?action=delete",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
	...
	$scope.edit = function(item){
		...
		$scope.modalTitle = dic('Edit',$rootScope.langSelected) + ' ' + dic('Branch',$rootScope.langSelected);
		$scope.modal = item;
	}
	...
	$scope.add = function(){
		...
		$scope.errorMessage = null;
		$scope.modalType = 'add';
		$scope.modalTitle = dic('Add',$rootScope.langSelected) + ' ' + dic('Branch',$rootScope.langSelected);
		$scope.modal = {};
	}
	...
	$scope.delete = function(item){
		...
		$scope.modalTitle = dic('Delete',$rootScope.langSelected) + ' ' + dic('Branch',$rootScope.langSelected);
		$scope.modal = item;
	}
