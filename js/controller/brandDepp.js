diakoApp.controller('brandDeppController',function($scope,$rootScope,$http,$log,$location,$routeParams,mainFactory,$indexedDB){
	...
	$scope.brandDeppList = null;
	$rootScope.part = 'brandDepp';
	...
	$scope.list = function(){
		dsh($rootScope.userDeppAuthSelect);
		if($rootScope.userDeppAuthSelect != undefined){
			$rootScope.loading = true;
			$http.get("control/brandCtrl.php?action=listDepp&order="+order+"&limit="+limitPage+"&search="+str+"&idDepp="+$rootScope.userDeppAuthSelect.id).
			success(function(data, status, headers, config) {
				$scope.brandDeppList = data.rows;
	...
	$scope.edit = function(item){
		...
		$scope.modalTitle = dic('Edit',$rootScope.langSelected) + ' ' + dic('brandDepp',$rootScope.langSelected);
		$scope.modal = item;
	}
	...
	$scope.add = function(){
		...
		$scope.errorMessage = null;
		$scope.modalType = 'add';
		$scope.modalTitle = dic('Add',$rootScope.langSelected) + ' ' + dic('brandDepp',$rootScope.langSelected);
		$scope.modal = {};
	}
	...
	$scope.delete = function(item){
		...
		$scope.modalTitle = dic('DElete',$rootScope.langSelected) + ' ' + dic('brandDepp',$rootScope.langSelected);
		$scope.modal = item;
	}
