diakoApp.controller('catController',function($scope,$rootScope,$http,$log,$location,$routeParams,mainFactory){
	...
	$scope.list = function(){
		...
		$http.get("control/catCtrl.php?action=list&order="+order+"&limit="+limitPage+"&search="+str).
		success(function(data, status, headers, config) {
			$scope.catList = data.rows;
	...
	$scope.saveModal = function(){
		if($scope.modalType == 'add'){
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/catCtrl.php?action=add",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
			...
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/catCtrl.php?action=edit",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
			...
			$rootScope.loading = true;
			$scope.lockForm = true;
			$http.post("control/catCtrl.php?action=delete",$scope.modal).
			success(function(data, status, headers, config) {
				if(data.result){
					$scope.modal = {};
					$('#appModal').modal('hide');
	...
	$scope.edit = function(item){
		...
		$scope.modalTitle = dic('edit_cat',$rootScope.langSelected);
		$scope.modal = item;
		dsh(item);
	}
	...
	$scope.add = function(){
		$scope.errorMessage = null;
		$scope.modalType = 'add';
		$scope.modalTitle = dic('add_cat',$rootScope.langSelected);
		$scope.modal = {};
	}
	...
	$scope.delete = function(item){
		...
		$scope.modalTitle = dic('delete_cat',$rootScope.langSelected);
		$scope.modal = item;
	}
